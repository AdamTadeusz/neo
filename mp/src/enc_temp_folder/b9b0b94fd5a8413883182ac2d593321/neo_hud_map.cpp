	...
	: CHudElement(pElementName), EditablePanel(parent, pElementName)
{
	...
	surface()->DrawSetTextureFile(m_hPlayerTexture, "../maps/minimaps/playerMapIcon", 1, false);
...
void CNEOHud_Map::FireGameEvent(IGameEvent* event)
{
...
void CNEOHud_Map::LoadMap()
{
	...
	if (mapNameLength <= 5)
	{ // map name should be prefixed with "maps/"
		return;
	}
...
void CNEOHud_Map::DrawPlayer(C_NEO_Player *player, Color colour) const
{
	...
	int playerX = (m_iY1 / 2) + (difference.y * m_flCos * m_flScale) + (difference.x * m_flCos * m_flScale);
	int playerY = (m_iY1 / 2) - (difference.y * m_flSin * m_flScale) + (difference.x * m_flSin * m_flScale) + difference.z * m_flScale;
...
void CNEOHud_Map::DrawMap() const
{
	...
	if (localPlayerTeam != TEAM_NSF)
	{
		...
		int memberCount = teamJinrai->GetNumPlayers();
	...
	if (localPlayerTeam != TEAM_JINRAI)
	{
		...
		int memberCount = teamNsf->GetNumPlayers();
		for (int i = 0; i < memberCount; ++i)
		{
